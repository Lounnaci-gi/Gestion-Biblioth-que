  ...
  try {
    ...
    const result = await pool.request().query(`
      SELECT
        ...
        (SELECT COUNT(*) FROM Adherents WHERE Statut = 'Actif') AS adherents_actifs,
        (SELECT COUNT(*) FROM Emprunts WHERE Statut = 'En cours') AS emprunts_en_cours,
        (SELECT COUNT(*) FROM Emprunts WHERE Statut = 'En cours' AND Date_Retour_Prévue < GETDATE()) AS emprunts_en_retard
    `);
